...
import PastryProductCard from '../comps/PastryProductCard';

import Footer from '../comps/Footer';

import { useRouter } from 'next/router';

export default function Pastry({ pastries }) {
    ...
    return <>
        ...
        <Box p={XS ? '.5rem' : '1.5rem'} className='cont'>
            ...
            <Grid container spacing={MD ? 2 : (XS ? 1 : 7)}>
                {pastries.map(pastry => <PastryProductCard {...pastry} key={pastry.id} />)}
            </Grid>
        </Box>
...
export async function getStaticProps() {
    ...
    const pastries = await fetch(`${origin}/data/pastries.json`);
    return {
        props: {
            pastries: await pastries.json(),
        }
    };
}